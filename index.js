...
app.listen(8000);
setInterval(() => {
  http.get(`http://shadowed-rhinestone-medicine.glitch.me/`);
}, 280000);
http
...
bot.on("ready", async () => {
  ...
  bot.user.setActivity("!help | discord.gg/robu");
});
...
bot.on('message', message => {
  ...
  if (cmd.admin && !message.guild.me.hasPermission([cmd.permissions_bot])) return message.channel.send("> **I don't have permission [" + `${cmd.permissions_bot.join(", ")}` + "]**");
  ...
  if (timestamps.has(message.author.id)) {
    ...
    if (now < expirationTime) {
      ...
      return message.channel.send(`> **please wait ${timeLeft.toFixed(1)} second(s)**`).then(msg => {
