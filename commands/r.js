...
module.exports.run = async(message, args, client) => {
  ...
  function price() {
    ...
    if (isNaN(args[1]) || parseInt(args[1]) < 1) return message.channel.send(lang.setprice.reply2);
  ...
  async function buy(){
   ...
   // if (!u1 && !u2) be = false;
    ...
    console.log(be)
    if (data.Status === false || be === false) {
      message.channel.send(lang.buy.reply9);
      cooldown.delete(col);
      return;
    }
      ...
      messages.on("collect",async collect => {
        ...
        if (count === 1){
          ...
          try {
            ...
            msg.edit(embed);
            mes = message.channel.createMessageCollector(user => user.author.id === `603761681717526538`, {time: 60000 * 2 });
              ...
              if (collect.content.includes(message.author.username) && collect.content.includes(`$${endprice}`) && collect.content.includes(`<@!${owner}>`) || collect.content.includes(`<@${owner}>`)){
                ...
                var Channel = message.guild.channels.cache.get(data.getThanksRoom);
                 ...
                 message.channel.send(lang.buy.reply8 + "\n" + tu);
                  ...
                  console.log(err)
                  message.channel.send("**الرجاء قراءة الرساله ادناه [mention]**\n\n".replace(/\[mention]/g,`<@${message.author.id}>`) + lang.buy.reply18.replace(/\[numberP]/g,`${numberP}`).replace(/\[username]/g,`${username}`));
                  message.channel.setName(`${numberP} robux`);
                  cooldown.delete(col);
                });
                }).catch(() => {
                  cooldown.delete(col);
